...
from classes.upgradewindow import upgrade_button, upgrade_window, cancel_upgrade_button, money_count
from classes.ButtonForUpgrade import Health_button, Reload_time_button, Bowled_count_button, Damage_button, \
...
def main():
    ...
    final = FinishWindow(all_sprites, final_image, 0, -700)
    ...
    health_count = HealthCount(all_sprites, take_info())
    upgrade = 1
    running = True
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                cancel_upgrade_button.check_click(event.pos, health_count, take_info())
        ...
        if start_button.button_pressed:
            ...
            health_count.update(time)
            health_count.draw(data.screen)
            ran_hero.set_move(horizontal)
            generate.do(time, game_speed, count.count)
            lets.update(time, game_speed, horizontal)
            data.birds.update(time, game_speed, horizontal)
            data.boss_group.update(time, game_speed, horizontal)
            ...
            chek_lets(ran_hero, finish_button, health_count, take_info(), generate)
